...
export function CommissionsPagination({
  ...
  const pages = getPaginationItems(currentPage, safeLastPage);

  return (
    <div className={cn("flex items-center justify-between border-t bg-muted/20 px-4 py-3 sm:px-6", isArabic && "flex-row-reverse")}>
      <p className="text-xs font-semibold text-muted-foreground">
        {isArabic ? (
          <>
            صفحة <span className="font-bold text-foreground tabular-nums">{currentPage}</span> من{" "}
            <span className="font-bold text-foreground tabular-nums">{safeLastPage}</span>
          </>
        ) : (
          <>
      ...
      <Pagination>
        <PaginationContent className={cn(isArabic && "flex-row-reverse")}>
          <PaginationItem>
            <PaginationPrevious href={buildHref(Math.max(1, currentPage - 1))} className={cn(currentPage <= 1 && "pointer-events-none opacity-50")} />
          </PaginationItem>
          ...
          <PaginationItem>
            <PaginationNext href={buildHref(Math.min(safeLastPage, currentPage + 1))} className={cn(currentPage >= safeLastPage && "pointer-events-none opacity-50")} />
          </PaginationItem>
        </PaginationContent>
      </Pagination>
    </div>
...
function getPaginationItems(currentPage: number, lastPage: number): (number | "ellipsis")[] {
  ...
  for (let page = Math.max(2, currentPage - 1); page <= Math.min(lastPage - 1, currentPage + 1); page++) pages.push(page);
