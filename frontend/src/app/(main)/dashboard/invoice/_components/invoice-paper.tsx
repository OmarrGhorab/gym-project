...
export function InvoicePaper({ invoice }: { invoice: InvoiceFormValues }) {
  ...
  const discountLabel =
    invoice.discountType === "percent" ? t("discountPercent", { value: discountValue }) : t("discount");
    ...
    >
      <header className="flex flex-col gap-10">
        <div className="grid grid-cols-2 items-start gap-14">
          ...
          <h2 className="text-4xl uppercase tracking-widest">{t("invoice")}</h2>
        </div>

        <section className="grid grid-cols-2 gap-14 text-sm leading-relaxed">
          <div>
            <p>{t("reference", { value: invoice.referenceNumber })}</p>
            <p>{t("issued", { value: invoice.issuedDate })}</p>
            <p>{t("paymentDue", { value: invoice.paymentDueDate })}</p>
          </div>
          <div>
            <p>{t("paymentAccount")}</p>
            <p>{invoice.from.paymentAccountName}</p>
            <p>{t("routingNumber", { value: invoice.from.routingNumber })}</p>
          </div>
        </section>

        <section className="grid grid-cols-2 gap-14 text-sm leading-relaxed">
          <div>
            <p className="mb-4 font-semibold uppercase">{t("from")}</p>
            <p>{invoice.from.name}</p>
            {invoice.from.addressLines.map((line) => (
              <p key={line}>{line}</p>
            ))}
            <p>{t("taxId", { value: invoice.from.taxId })}</p>
          </div>
          <div>
            <p className="mb-4 font-semibold uppercase">{t("billTo")}</p>
            <p>{invoice.to.name}</p>
            {invoice.to.addressLines.map((line) => (
              <p key={line}>{line}</p>
            ))}
            <p>{t("taxId", { value: invoice.to.taxId })}</p>
          </div>
        </section>
      </header>
      ...
      <div className="flex flex-col gap-5">
        <section className="text-sm">
          <div className="grid grid-cols-[1fr_74px_116px_116px] bg-stone-200 px-3 py-3 font-semibold uppercase">
            ...
            <span className="text-right">{t("units")}</span>
        ...
        <div className="grid grid-cols-2 gap-14 text-sm leading-relaxed">
          <section className="col-start-2 space-y-2">
            ...
            <div className="border-current border-y-2 py-3">
              <div className="flex justify-between gap-8">
                <span className="font-semibold uppercase">{t("balanceDue")}</span>
      ...
      <footer className="absolute right-12.25 bottom-11 left-12.25 grid grid-cols-2 gap-14 text-neutral-500 text-sm leading-relaxed">
        ...
        <div>
          <p>{t("prepared")}</p>
          <p>{t("issuedBy", { name: invoice.from.issuerName })}</p>
        </div>
      </footer>
    </article>
  );
